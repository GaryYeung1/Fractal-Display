        ...
        setWorkers.addActionListener(new ActionListener(){
        	public void actionPerformed(ActionEvent evt){
        		...
        		for(int i = 0; i <= desiredWorkers%2048; i++){
        			workerArray[i] = new SwingWorkerHandler(i*((2048/desiredWorkers) +1),2048/desiredWorkers+1,escapeSteps,_userEscapeTime);
        		}
        		for(int j = desiredWorkers%2048+1; j < desiredWorkers; j++){
        			workerArray[j] = new SwingWorkerHandler(j*(2048/desiredWorkers), 2048/desiredWorkers,escapeSteps, _userEscapeTime);
        		}
        	}
        });
